...
class getid3_tak extends getid3_handler {
	...
	public function Analyze() {
		...
		while ( $objtype != 0 ) {
			switch ( $objtype ) {
				...
				case 3:
					...
					if ( $headersize ) {
						$thisfile_takaudio_raw['header_data'] = substr( $TAKMetaData, 6, $headersize );
					}
					if ( $footersize ) {
						$thisfile_takaudio_raw['footer_data'] = substr( $TAKMetaData, $headersize, $footersize );
					}
					$offset += $objlength;
					break;
				default:
		...
		foreach ( [ 'bits_per_sample', 'version', 'profile' ] as $key ) {
