...
class getid3_torrent extends getid3_handler {
	...
	public function Analyze() {
		...
		if ( ! empty( $info['torrent']['info']['length'] ) && ! empty( $info['torrent']['info']['piece length'] ) && ! empty( $info['torrent']['info']['pieces'] ) ) {
			...
			$num_pieces_hash = strlen( $info['torrent']['info']['pieces'] ) / self::PIECE_HASHLENGTH; // should be concatenated 20-byte SHA1 hashes
			if ( $num_pieces_hash == $num_pieces_size ) {
				$info['torrent']['info']['piece_hash'] = [];
				for ( $i = 0; $i < $num_pieces_size; $i++ ) {
					$info['torrent']['info']['piece_hash'][ $i ] = '';
					for ( $j = 0; $j < self::PIECE_HASHLENGTH; $j++ ) {
						$info['torrent']['info']['piece_hash'][ $i ] .= sprintf( '%02x', ord( $info['torrent']['info']['pieces'][ ( ( $i * self::PIECE_HASHLENGTH ) + $j ) ] ) );
					}
				}
		...
		if ( ! empty( $info['torrent']['info']['name'] ) && ! empty( $info['torrent']['info']['length'] ) && ! isset( $info['torrent']['info']['files'] ) ) {
			// single-file torrent
			$info['torrent']['files'] = [ $info['torrent']['info']['name'] => $info['torrent']['info']['length'] ];
