  ...
  var blocks = blockMap.map(function (block) {
    ...
    var chars = block.getCharacterList().map(function (char) {
      ...
      if (entityKey) {
        ...
        if (shouldRemove) {
          altered = true;
          return draftJs.CharacterMetadata.applyEntity(char, null);
        }
      }
...
/*: ContentState*/
  ...
  if (nextContent.getBlockMap().size === 0) {
    return draftJs.EditorState.moveFocusToEnd(draftJs.EditorState.set(editorState, {
      currentContent: draftJs.ContentState.createFromText("")
    }));
  }

  var nextState = draftJs.EditorState.set(editorState, {
